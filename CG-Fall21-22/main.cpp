...
/*void intro()
{
    if(isIntro==true)
    {
        ...
        glEnd();
        glColor3f(1.0 , 1.0, 1.0);
        char *C="A RIVERSIDE BUSY ROAD";
        char *P="PROJECT DONE BY:";
        char *F="CHOWDHURY, MD. FAHIM KABIR";
        char *N="NAWSHAD, NADIM";
        char *S="KABIR, SHAMIMA";
        char *M="BHUYIAN, MD. MOHITUZZAMAN";
        char *X="SUBMITTED TO:";
        char *FS="FAISAL, MD SAJID BIN";
        ...
        glRasterPos2f(  -0.3 , 0.49  );
        for(int i = 0; N[i] !='\0'; i++)
        {
             glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, N[i]);
        }
        glRasterPos2f( -0.3 , 0.44 );
        for(int i = 0; S[i] !='\0'; i++)
        {
             glutBitmapCharacter(GLUT_BITMAP_TIMES_ROMAN_24, S[i]);
        }
        glRasterPos2f( -0.3 , 0.39 );
        for(int i = 0; M[i] !='\0'; i++)
        {
...
int main(int argc, char** argv){
    ...
    glutCreateWindow ("Group-9's Project");
